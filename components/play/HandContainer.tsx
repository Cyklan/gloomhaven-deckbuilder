...
export const HandContainer: FC<HandContainerProps> = ({ cards, prefix, availableCardActions, selectedCards, turnState, setTurnState, showBottomButton }) => {
  ...
  const cardActionButtons = availableCardActions.map(action => <button
    onClick={() => {
    ...
    {action.title === "Pick" ? selectedCards.filter(x => x.title === selectedCard?.title).length > 0 ? "Remove" : "Pick" : action.title}
  </button>);
    ...
    {turnState !== TurnState.ENDING && <>
      ...
      <div className="modal">
        <div className="modal-box">
          <label onClick={() => modalToggleRef.current?.click()} htmlFor="large-card-view" className="btn btn-square absolute right-2 top-2 z-10">
            <Close />
          </label>
          <div className="relative flex flex-col h-full">
            {selectedCard && (
              <>
                ...
                <Card card={selectedCard} imagePath={`/cards/${prefix}/${selectedCard.imgName}`} />
                {turnState !== TurnState.PLAYING && <div className="flex-auto flex flex-col-reverse items-center">
                  {cardActionButtons}
                </div>}
              </>
            )}
