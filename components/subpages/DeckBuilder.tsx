...
export const DeckBuilder: FC<DeckBuilderProps> = ({
  character,
  deckName,
  closeDeckBuilder,
  save,
  ...
  const [deck, setDeck] = useState<CardModel[]>(
    (editDeck && convertSavedDeck(editDeck).cards) || []
  );
  ...
  function removeFromDeck(card: CardModel) {
    ...
    setUnusedCards([...unusedCards].sort((a, b) => a.level - b.level));
  ...
  const cardViewCards = unusedCards.map((x) => {
    const levelIsAlreadyUsed =
      x.level > 1 &&
      deck.filter((y) => x.level === y.level).length > 0 &&
      !deck.includes(x);
    const isUsed = deck.filter((y) => y.imgName === x.imgName).length > 0;
    const maxCardsUsed = deck.length === character.handLimit;
    const infoTexts: Array<string> = [];

    if (maxCardsUsed) infoTexts.push("Deck limit reached");
    if (levelIsAlreadyUsed) infoTexts.push("Level already added");
    if (isUsed) infoTexts.push("Card already added");
    ...
    return (
      <Card
        ...
        className={
          levelIsAlreadyUsed || isUsed || maxCardsUsed ? "grayscale" : ""
        }
        infoTexts={infoTexts}
      />
    );
    ...
    <>
      <div
        className={`w-screen min-h-screen bg-base-200 ${
          sidebarOpen ? "overflow-hidden max-h-screen" : ""
        }`}
      >
        <div className="flex w-full justify-around items-center h-24 bg-base-200 fixed top-0 z-10">
          ...
          <div
            className={`text-xl aspect-square w-12 flex items-center justify-center ${
              deck.length === character.handLimit ? "text-red-500" : ""
            }`}
          >
          ...
          <div className="relative flex flex-col h-full">
            {selectedCard && (
              <>
                ...
                <Card
                  card={selectedCard}
                  imagePath={`/cards/${character.prefix}/${selectedCard.imgName}`}
                />
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      (selectedCard.level > 1 &&
                        deck.filter((x) => x.level === selectedCard.level)
                          .length > 0 &&
                        !deck.includes(selectedCard))
                    }
                    onClick={() => {
